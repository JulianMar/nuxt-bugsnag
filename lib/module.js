...
module.exports = function (options) {
  ...
  if (bugsnagOptions.options) {
    bugsnagOptions.browserOptions = bugsnagOptions.options
    bugsnagOptions.serverOptions = bugsnagOptions.options
  }

  logger.info('Adding browser plugin')
  this.addPlugin({
    src: join(__dirname, 'plugin.js'),
    options: { apiKey: bugsnagOptions.browserApiKey, ...bugsnagOptions.browserOptions },
    ssr: false
  })

  const bugsnagClient = bugsnag({ apiKey: bugsnagOptions.serverApiKey, logger, ...bugsnagOptions.serverOptions })
